...
vector<int64_t> divisors(int64_t x){
    assert(x <= int(1e12) && "prime_factorize(x) is only valid upto sqrt(x) <= 1e6");
...
constexpr int sieve_max = 1000;

// this method is somewhat general so it can work in any case

int number_of_time(int n, int k){
    int res = 0;
    while(n){
        n /= k;
        res += n;
...
int main(){
    ...
    auto f = prime_factorize(k);

    int ans = INT_MAX;

    for(auto &[ki, p] : f){
        int a = number_of_time(n, ki);
        ans = min(ans, (a/p));
    }
    cout << ans << '\n';
